...
class Dbhelper {
  ...
  Future<Database?> createDatabase() async {
    ...
    _database = await openDatabase(
      ...
      onCreate: (db, version) async {
        ...
        String userSql =
        '''CREATE TABLE user(id INTEGER PRIMARY KEY AUTOINCREMENT,name TEXT,email TEXT,phone TEXT,image BLOB) ''';
  ...
  Future<void> insertRecords(
      double amount, int isIncome, String date, String category) async {
    Database? db = await database;

    String query =
        '''INSERT INTO budget(amount,isIncome,date,category) VALUES(?,?,?,?)''';
  ...
  Future<void> updateRecords(
      double amount, int isIncome, String date, String category, int id) async {
    Database? db = await database;
    String query =
        '''UPDATE budget SET amount=?,isIncome=?,date=?,category=? WHERE id=?''';
    List argumnet = [amount, isIncome, date, category, id];
    await db!.rawUpdate(query, argumnet);
  }

  Future<List<Map<String, Object?>>> fetchDatafromFilter(int isIncome) async {
    Database? db = await database;
  ...
  Future<List<Map<String, Object?>>> filterBySeacrh(String search) async {
    Database? db = await database;
    String query = '''SELECT * FROM budget WHERE category LIKE '$search%' ''';
    return await db!.rawQuery(query);
  }
}
